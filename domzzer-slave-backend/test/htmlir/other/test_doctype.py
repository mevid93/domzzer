...
class TestHTMLDoctype(unittest.TestCase):
    ...
    def test_constructor_works_without_parameters(self):
        doctype = HTMLDoctype()
        self.assertEqual(doctype.document_type, 0)

    def test_constructor_works_with_parameter(self):
        doctype = HTMLDoctype(20)
        self.assertEqual(doctype.document_type, 20)
    ...
    def test_generate_works(self):
        ...
        self.assertTrue(doctype.document_type >= 0 and doctype.document_type <= 20)

    def test_mutate_works(self):
        random.seed(666)
        doctype = HTMLDoctype()
        doctype.mutate()
        self.assertTrue(doctype.document_type != 0)
